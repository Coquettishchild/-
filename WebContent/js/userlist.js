...
function getlist() {
	...
	xhr2.open("post", "../getuserlist.action", true);
		...
		if (xhr2.readyState == 4 && xhr2.status == 200) {
			...
			if (json.flag && json.obj.length!=0) {
				for (j = 0; j < json.obj.length; j++) {
					ids[j] = json.obj[j].id;
					tittle[j].innerHTML = json.obj[j].username;
					createdata[j].innerHTML = json.obj[j].sex;
					count[j].innerHTML = json.obj[j].email;
...
for (let j = 0; j < 10; j++) {
	deletea[j].onclick = function() {
		...
		var xhr = createxmlreuqest();
		xhr.open("post", "../deleteuser.action", true);
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				try {
					...
					alert("请先登录");
					window.location.href = '../secendthml/admin.html';
				}
...
for(let k=0;k<10;k++){
	tittle[k].onclick=function(){
		...
		tittle[k].href="../secendhtml/showuser.html?id="+id[k];
	}
}
